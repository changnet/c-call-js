...
## 重置工程
如果发现文件缺失或者错误（通常是gclient中断过）可以用以下命令修复（这个并不会重新下载所有文件，会比较快）
```bash
git reset --hard HEAD
git checkout 13.6.233.17
...
## 以库方式构建
v8本身设计为嵌入其他工程（比如chrome或者nodejs），因此本身默认的构建参数编译出来的`v8_monolith.a`是不能链接到一个so的，只能链接到一个程序。

```
g++ -shared -fPIC -o v8.so src/v8.cpp -std=c++20 -O2 -fno-rtti -I./v8 -I./v8/include -DV8_COMPRESS_POINTERS -Lv8/lib -lv8_monolith -llua -fuse-ld=lld -lpthread -ldl -latomic -Wl,-Bsymbolic
ld.lld: error: relocation R_X86_64_TPOFF32 against v8::internal::g_current_isolate_ cannot be used with -shared
>>> defined in v8/lib/libv8_monolith.a(isolate.o)
>>> referenced by api.cc
>>>               api.o:(v8::internal::V8::FatalProcessOutOfMemory(v8::internal::Isolate*, char const*, v8::OOMDetails const&)) in archive v8/lib/libv8_monolith.a
```

但我是需要以lua的动态库运行，因此得手动修改参数。

在chrome或者nodejs的构建中，都是有参数可以指定v8是否以为方式构建的

https://chromium.googlesource.com/v8/v8/+/refs/heads/main/BUILD.gn

https://github.com/nodejs/node/blob/main/tools/v8_gypfiles/v8.gyp

这里都可以指定`V8_TLS_USED_IN_LIBRARY`，但v8本身的构建里我就搜索不到相关参数。

不管了，直接修改`src\common\thread-local-storage.h`，把所有分支都定义`#define V8_TLS_LIBRARY_MODE 1`

## 构建
使用官方的方法（https://v8.dev/docs/build）构建不太行，因为不是最新。并且程序后面需要和gcc程序链接。下面是一些常用参数解析
```
...
注意，这个`is_debug = false`不会生成pdb文件，所有调试符号全部写入lib文件，默认编译出来的v8_monolith.lib大小为1.9G，加了
```
v8_symbol_level = 0
symbol_level = 0
```
